        ...
        <div class="card bg-light-secondary shadow-none position-relative overflow-hidden">
            <div class="card-body px-4 py-3">
                <div class="row align-items-center">
                    <div class="col-9">
                        <h4 class="fw-semibold mb-8">Edit Data Pengumuman</h4>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a class="text-muted"
                                        href="{{ route('admin-panel.dashboard') }}">Dashboard</a></li>
                                <li class="breadcrumb-item"><a class="text-muted"
                                        href="{{ route('admin-panel.announcement.index') }}">Data Pengumuman</a></li>
                                <li class="breadcrumb-item" aria-current="page">Edit Data Pengumuman</li>
                            </ol>
                        </nav>
                    </div>
                    <div class="col-3">
                        <div class="text-center">
                            <img src="{{ asset('panel-assets/dist/images/breadcrumb/announcement.png') }}" alt=""
                                class="img-fluid">
                        </div>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header bg-warning">
                        <h4 class="mb-0 text-white card-title">Masukkan Data Pengumuman Disini</h4>
                    </div>
                    <div class="card-body">
                        @if (count($errors) > 0)
                        ...
                        @endif
                        <form method="POST" action="{{ route('admin-panel.announcement.update', $announcement->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="d-flex justify-content-center">
                                <img src="{{ asset($announcement->cover_image) }}" class="img-fluid"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="status" id="status" class="form-select">
                                    <option value="published" {{ $announcement->status == 'published' ? 'selected' : '' }}>Dipublikasikan</option>
                                    <option value="drafted" {{ $announcement->status == 'drafted' ? 'selected' : '' }}>Diarsipkan</option>
                                </select>
                            </div>
                            <button type="submit" class="btn btn-success">Simpan</button>
                            <a href="{{ route('admin-panel.announcement.index') }}" class="btn btn-warning mx-2">Kembali</a>
                        </form>
                    </div>
                </div>
            </div>
